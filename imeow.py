...
def evaluate_actions(policy, obs, actions) -> tuple[Any, Any, Any]:
    """
...
@dataclasses.dataclass(frozen=True)
class InverseMEowLossCalculator:
    ...
    def __call__(
        ...
        Returns:
            A IMEowTrainingMetrics object with the loss and all the components it
            consists of.
        """
...
class IMEowLogger:
    ...
    def log_batch(
            ...
            training_metrics: IMEowTrainingMetrics,
            rollout_stats: Mapping[str, float],
    ):
        self._logger.record("batch_size", batch_size)
        self._logger.record("IMEow/epoch", self._current_epoch)
        self._logger.record("IMEow/batch", batch_num)
        self._logger.record("IMEow/samples_so_far", num_samples_so_far)
        for k, v in training_metrics.__dict__.items():
            self._logger.record(f"IMEow/{k}", float(v) if v is not None else None)
...
class IMEow(algo_base.DemonstrationAlgorithm):
    """Behavioral cloning (IMEow).
    ...
    def __init__(
        ...
        self._IMEow_logger = IMEowLogger(self.logger)
    ...
    def train(
        ...
        if reset_tensorboard:
            self._IMEow_logger.reset_tensorboard_steps()
        self._IMEow_logger.log_epoch(0)
        ...
        def _on_epoch_end(epoch_number: int):
            ...
            self._IMEow_logger.log_epoch(epoch_number + 1)
